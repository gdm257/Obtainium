...
void main() {
  group('cloudBackupFileName', () {
    test(
      'mirrors the local export name: prefix + ISO8601 with ":" -> "-" + .json',
      () {
        final name = cloudBackupFileName(
          prefix: 'obtainx-export',
          at: DateTime(2026, 7, 27, 9, 30, 5, 123, 456),
        );
        expect(name, 'obtainx-export-2026-07-27T09-30-05.123456.json');
      },
    );
  ...
  group('filterCloudCredsForImport', () {
    ...
    test(
      'leaves non-cloud creds alone (they follow normal import overwrite)',
      () {
        final backup = {
  ...
  group('applyCredsImportRule', () {
    test(
      'strips a device-owned cloud cred so import keeps the device value',
      () {
        const backup =
        ...
        expect(
          settings.containsKey('cloudBackupS3_secretAccessKey-creds'),
          isFalse,
        );
    ...
    test('keeps a backup cloud cred when the device has none', () {
      ...
      final settings =
          (jsonDecode(out) as Map)['settings'] as Map<String, dynamic>;
